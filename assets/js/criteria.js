...
(function (global) {
  ...
  function describe(cfg, ctx) {
    ...
    if (cfg.key === 'concentration_cutoff') {
      ...
      return colTxt + ' < ' + (cut === null || cut === undefined || isNaN(cut) ? 'cut-off' : U.fmtNum(cut, 4) + ' ng/mL') + ' → 0';
    }
   ...
   * → { total, passed, failed, warnings, transformed, skipped, rows:[…], byCriterion:{} }
   */
  function process(rows, streamOf, criteria, ctx) {
    var enabled = (criteria || []).filter(function (c) { return c.enabled; });
    var out = {
      total: rows.length, passed: 0, failed: 0, warnings: 0, transformed: 0,
      rows: [], byCriterion: {}, notMapped: []
    };
    enabled.forEach(function (c) {
      out.byCriterion[c.key] = { evaluated: 0, failed: 0, warnings: 0, transformed: 0, skipped: 0 };
    ...
    rows.forEach(function (row) {
      ...
      record.status = record.failures.length ? 'fail' : (record.warnings.length ? 'warning' : 'pass');
      if (record.status === 'pass') out.passed++;
      else if (record.status === 'fail') out.failed++;
      else out.warnings++;
      if (record.transforms.length) out.transformed++;
      out.rows.push(record);
    });
    return out;
  }

  /** Row values after the criteria transformations (e.g. sub-cut-off → 0). */
  function appliedRow(record) {
    if (!record.transforms.length) return record.row;
    var copy = Object.assign({}, record.row);
    record.transforms.forEach(function (t) { copy[t.column] = t.value; });
    return copy;
  }
